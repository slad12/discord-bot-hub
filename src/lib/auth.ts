...
export const signInWithDiscord = async () => {
  ...
  const functionUrl = `${supabaseUrl}/functions/v1/discord-auth?action=login&redirect_uri=${encodeURIComponent(redirectUri)}`;
...
export const handleDiscordCallback = async (code: string) => {
  ...
  const functionUrl = `${supabaseUrl}/functions/v1/discord-auth?action=callback`;
